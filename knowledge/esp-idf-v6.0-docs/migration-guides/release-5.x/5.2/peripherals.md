<!-- Source: _sources/migration-guides/release-5.x/5.2/peripherals.rst.txt (ESP-IDF v6.0 documentation) -->
...
I2C driver has been redesigned (see `I2C API Reference <../../../api-reference/peripherals/i2c>`), which aims to unify the interface and extend the usage of I2C peripheral. Although it is recommended to use the new driver APIs, the legacy driver is still available in the previous include path `driver/i2c.h`.
...
- `i2c_rw_t` which was used to tell whether I2C master controller is performing a <span class="title-ref">write</span> or a <span class="title-ref">read</span> operation. This is now deprecated.
