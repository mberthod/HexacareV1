<!-- Source: _sources/migration-guides/release-5.x/5.2/system.rst.txt (ESP-IDF v6.0 documentation) -->
...
- When enabling single-core mode via the `CONFIG_FREERTOS_UNICORE` option, the kernel's behavior will now be identical to Vanilla FreeRTOS (see `freertos-idf-single-core` for more details).
