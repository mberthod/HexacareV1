<!-- Source: _sources/api-reference/api-conventions.rst.txt (ESP-IDF v6.0 documentation) -->
...
- Build system functions, predefined variables, and options. These are documented in the `ESP-IDF CMake Build System API <cmake_buildsystem_api>`.
- `Kconfig options <configs-in-C-Cmake>` can be used in code and in the build system (`CMakeLists.txt`) files.
- `Host tools <../api-guides/tools/index>` and their command line parameters are also part of the ESP-IDF interfaces.
...
Most ESP-IDF APIs return error codes defined with the `esp_err_t` type. See `Error Handling <../api-guides/error-handling>` section for more information about error handling approaches. `Error Codes Reference <error-codes>` contains the list of error codes returned by ESP-IDF components.
...
Most ESP-IDF examples use C99 [designated initializers](https://en.cppreference.com/w/c/language/struct_initialization) for structure initialization since they provide a concise way of setting a subset of fields, and zero-initializing the remaining fields:
...
Certain header files in ESP-IDF contain APIs intended to be used only in ESP-IDF source code rather than by the applications. Such header files often contain `private` or `esp_private` in their name or path. Certain components, such as `hal <../api-guides/hardware-abstraction>` only contain private APIs.
...
ESP-IDF uses [Semantic Versioning](https://semver.org/) as explained in the `Versioning Scheme <versioning-scheme>`.
...
- Renaming Kconfig options. Kconfig system's `backward compatibility <configuration-options-compatibility>` ensures that the original Kconfig option names can still be used by the application in `sdkconfig` file, CMake files, and source code.
...
- Adding new structure members or changing the order of members. See `api_reference_config_structures` for tips that help ensure compatibility.
...
- `api_reference_private_apis`.
- `api_reference_example_components`.
...
- Location of `Kconfig <project-configuration-guide>` options in menuconfig.
