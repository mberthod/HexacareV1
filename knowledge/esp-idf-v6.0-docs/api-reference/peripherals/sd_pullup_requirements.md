<!-- Source: _sources/api-reference/peripherals/sd_pullup_requirements.rst.txt (ESP-IDF v6.0 documentation) -->
...
<!-- Only for: esp32 -->
By default, the MTDI bootstrapping pin is incompatible with the DAT2 line pull-up if the flash voltage is 3.3 V. For more information, see `mtdi_strapping_pin` below.

Add a diagram of the bus lines and pullups

This document has the following structure:

- `compatibility_overview_espressif_hw_sdio` between the default pull states on pins of Espressif's products and the states required by the SD bus
- `sdio_solutions` - ideas on how to resolve compatibility issues
- `related_info_sdio` - other relevant information
...
- ESP32 (except for D2WD versions, see [ESP32 datasheet](https://www.espressif.com/sites/default/files/documentation/esp32_datasheet_en.pdf)):

  > - `sd_pull-up_no_pull-ups`
  > - `strapping_conflicts_dat2` for models with 3.3 V flash chip

- ESP32-D2WD:

  > - `sd_pull-up_no_pull-ups`
  > - `no_pull-up_on_gpio12`
...
- ESP-WROVER-KIT
  ...
  > - `pull-up_conflicts_on_gpio13` (v4.1, v3, v2, and v1)
  > - `strapping_conflicts_dat2` (v4.1, v2, and v1)
  > - `gpio2_strapping_pin` (v2, v1)
  >
...
- If your host and slave device are on separate boards, replace one of them with a board that has pull-ups. For the list of Espressif's development boards with pull-ups, go to `sdio_dev_kits`.
- Attach external pull-ups by connecting each pin which requires a pull-up to VDD via a 10 kΩ resistor.

<!-- Only for: not esp32 and not esp32s3 -->
When using a development board without pull-ups:
...
There is a conflict between the boot strapping requirements of the ESP32 and the SDIO protocol. For details, see `mtdi_strapping_pin`.
...
For more details, see **{IDF_TARGET_NAME} Technical Reference Manual** \[[PDF](%7BIDF_TARGET_TRM_EN_URL%7D#efuse)\].
...
- On boards using the internal regulator and a 3.3 V flash chip, GPIO12 must be pulled low at reset. This is incompatible with the SD card operation. For reference information on compatibility of Espressif's boards with the SD card operation, see `compatibility_overview_espressif_hw_sdio`.
