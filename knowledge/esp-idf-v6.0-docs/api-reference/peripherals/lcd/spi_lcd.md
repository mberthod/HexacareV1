<!-- Source: _sources/api-reference/peripherals/lcd/spi_lcd.rst.txt (ESP-IDF v6.0 documentation) -->

# SPI Interfaced LCD

1.  Create an SPI bus. Please refer to `SPI Master API doc </api-reference/peripherals/spi_master>` for more details.
...
2.  Allocate an LCD IO device handle from the SPI bus. In this step, you need to provide the following information:
    ...
    > - `esp_lcd_panel_io_spi_config_t::spi_mode` sets the SPI mode. The LCD driver uses this mode to communicate with the LCD. For the meaning of the SPI mode, please refer to the `SPI Master API doc </api-reference/peripherals/spi_master>`.
