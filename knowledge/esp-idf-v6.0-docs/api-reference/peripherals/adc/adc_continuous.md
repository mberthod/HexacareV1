<!-- Source: _sources/api-reference/peripherals/adc/adc_continuous.rst.txt (ESP-IDF v6.0 documentation) -->
...
To perform single, on-demand ADC conversions, check `ADC Oneshot Mode Driver <adc_oneshot>`.

## Driver Concepts

ADC continuous mode organizes data into conversion frames, which consist of multiple conversion results.

- Conversion Frame: A frame contains multiple conversion results. Conversion frame size is configured with `adc_continuous_new_handle` in bytes.
- Conversion Result: A result consists of multiple bytes, as defined by `SOC_ADC_DIGI_RESULT_BYTES`. Its structure is `adc_digi_output_data_t`, including the ADC unit, ADC channel, and raw data.

<img src="/../_static/diagrams/adc/adc_conversion_frame.png" class="align-center" alt="image" />
...
- `adc-continuous-resource-allocation`: covers which parameters should be set up to initialize the ADC continuous mode driver and how to deinitialize it.
- `adc-continuous-adc-configurations`: describes how to configure the ADC(s) to make it work under continuous mode.
- `adc-continuous-adc-control`: describes ADC control functions.
- `adc-continuous-register-event-callbacks`: describes how to hook user-specific code to an ADC continuous mode event callback function.
- `adc-continuous-read-conversion-result`: covers how to get ADC conversion result.
- `adc-continuous-hardware-limitations`: describes the ADC-related hardware limitations.
- `adc-continuous-power-management`: covers power management-related information.
- `adc-continuous-iram-safe`: covers the IRAM safe functions.
- `adc-continuous-thread-safety`: lists which APIs are guaranteed to be thread-safe by the driver.
...
- `adc_continuous_handle_cfg_t::max_store_buf_size`: set the maximum size of the pool in bytes, and the driver saves ADC conversion result into the pool.
...
<!-- Only for: esp32 -->
Especially, when this function returns `ESP_ERR_NOT_FOUND`, this means the I2S0 peripheral is in use. See `adc-continuous-hardware-limitations` for more information.

<!-- Only for: esp32s2 -->
Especially, when this function returns `ESP_ERR_NOT_FOUND`, this means the SPI3 peripheral is in use. See `adc-continuous-hardware-limitations` for more information.
...
not SOC_ADC_DIG_IIR_FILTER_UNIT_BINDED
...
- `adc_continuous_monitor_register_event_callbacks`: register user callbacks to take action when the ADC value exceeds of the thresholds.
...
First, add a DC bias to the input signal through a circuit to "shift" the negative signal into the ADC's measurement range. For the measurement range, please refer to the On-Chip Sensor and Analog Signal Processing chapter in [TRM](%7BIDF_TARGET_TRM_EN_URL%7D). For example, adding a 1 V bias would transform a signal from -1 V to +1 V into 0 V to 2 V range. Then by setting the appropriate high and low thresholds, the ADC can detect if the input signal approaches zero, allowing for the identification of phase changes in the signal. Refer to the example code below for details.
...
- `adc_digi_pattern_config_t::atten`: ADC attenuation. Refer to the On-Chip Sensor and Analog Signal Processing chapter in [TRM](%7BIDF_TARGET_TRM_EN_URL%7D).
...
- If there is no conversion result generated in the internal pool, the function will block for `timeout_ms` until at least one conversion result is generated. If there are still no generated results, the function will return `ESP_ERR_TIMEOUT`.
...
| Vmax | Maximum measurable input analog voltage, this is related to the ADC attenuation, please refer to the On-Chip Sensor and Analog Signal Processing chapter in [TRM](%7BIDF_TARGET_TRM_EN_URL%7D#sensor). |
...
To do further calibration to convert the ADC raw result to voltage in mV, please refer to `adc_calibration`.

#### Parse ADC Raw Data

The raw data read from ADC continuous mode needs to be further parsed to obtain usable ADC conversion results. The function `adc_continuous_parse_data` provides the functionality to parse raw data into structured ADC data.
...
- **Length alignment**: <span class="title-ref">raw_data_size</span> must be a multiple of `SOC_ADC_DIGI_RESULT_BYTES`
- **Buffer size**: Ensure the <span class="title-ref">raw_data</span> buffer is large enough to hold <span class="title-ref">raw_data_size</span> bytes of data
...
The parsed data structure `adc_continuous_data_t` contains the following information:

- `adc_continuous_data_t::unit`：ADC unit (ADC_UNIT_1 or ADC_UNIT_2)
- `adc_continuous_data_t::channel`：ADC channel number (0-9)
- `adc_continuous_data_t::raw_data`：ADC raw data value (0-4095, 12-bit resolution)
- `adc_continuous_data_t::valid`：Whether the data is valid

#### Read and Parse ADC Data

To simplify the usage flow, the function `adc_continuous_read_parse` is provided, which merges the read and parse operations into a single function call.
...
### Hardware Limitations<span id="adc-continuous-hardware-limitations"></span>
...
ADC continuous mode driver APIs are not guaranteed to be thread-safe. However, the share hardware mutual exclusion is provided by the driver. See `adc-continuous-hardware-limitations` for more details.

## Application Examples

- `peripherals/adc/continuous_read` demonstrates how to use the ADC Continuous Read Mode (DMA Mode) on {IDF_TARGET_NAME} development boards to read from GPIO pins via on-chip ADC modules.
