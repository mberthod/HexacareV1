<!-- Source: _sources/api-reference/peripherals/ledc.rst.txt (ESP-IDF v6.0 documentation) -->
...
<figure>
<img src="../../../_static/ledc-api-settings.jpg" class="align-center" alt="../../../_static/ledc-api-settings.jpg" />
<figcaption>Key Settings of LED PWM Controller's API</figcaption>
</figure>
...
<!-- Only for: esp32c6 or esp32c61 or esp32p4 -->
| Clock name  | Clock freq | Clock capabilities                                           |
|-------------|------------|--------------------------------------------------------------|
| PLL_80M_CLK | 80 MHz     | /                                                            |
| RC_FAST_CLK | ~ 17.5 MHz | Dynamic Frequency Scaling compatible, Light-sleep compatible |
...
Another way to set the duty cycle, as well as some other channel parameters, is by calling `ledc_channel_config` covered in Section `ledc-api-configure-channel`.
...
<!-- Only for: esp32 or esp32s2 or esp32s3 or esp32c3 or esp32c2 or esp32c6 or esp32h2 or esp32p4 -->
> **Warning**
>
> >
> Warning
...
The first function is called "behind the scenes" by `ledc_timer_config` to provide a startup of a timer after it is configured.

SOC_LEDC_SUPPORT_ETM and SOC_ETM_SUPPORTED

## LEDC's ETM Events and Tasks

LEDC can generate various events that can be connected to the `ETM </api-reference/peripherals/etm>` module. Timer's supported events are listed in `ledc_timer_etm_event_type_t`, and channel's supported events are listed in `ledc_channel_etm_event_type_t`. Users can create an `ETM event` handle by calling `ledc_timer_new_etm_event` or `ledc_channel_new_etm_event` respectively. LEDC also supports some tasks that can be triggered by other events and executed automatically. Timer's supported tasks are listed in `ledc_timer_etm_task_type_t`, and channel's supported tasks are listed in `ledc_channel_etm_task_type_t`. Users can create an `ETM task` handle by calling `ledc_timer_new_etm_task` or `ledc_channel_new_etm_task` respectively.
...
\* `peripherals/ledc/ledc_fade` demonstrates how to control the intensity of LEDs using the LEDC fade functionality. :SOC_LEDC_GAMMA_CURVE_FADE_SUPPORTED: \* `peripherals/ledc/ledc_gamma_curve_fade` demonstrates how to use the LEDC for color control of RGB LEDs with gamma correction. :SOC_LEDC_SUPPORT_ETM and SOC_ETM_SUPPORTED: \* `peripherals/ledc/ledc_dimmer` demonstrates how to use the LEDC and ETM to generate TRIAC gate trigger pulses that are synchronized to the mains zero‑cross.
