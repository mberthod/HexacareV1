<!-- Source: _sources/api-reference/storage/nvs_bootloader.rst.txt (ESP-IDF v6.0 documentation) -->
...
The API also supports decrypting NVS data, if the NVS partition is encrypted using one of the schemes as mentioned in the `nvs_encryption` guide.

Applications are expected to follow the steps below in order to enable decryption of a NVS partition using the `nvs_bootloader_read` API:

> 1.  Populate the NVS security configuration structure `nvs_sec_cfg_t` according to the selected NVS encryption scheme (Please refer to `nvs_encryption` for more details).
> 2.  Read NVS security configuration set by the specified security scheme using the `nvs_bootloader_read_security_cfg` API.
> 3.  Initialise the NVS flash partition with the above read security configuration using the `nvs_bootloader_secure_init` API.
> 4.  Perform NVS read operations using the `nvs_bootloader_read` API.
> 5.  Deinitialise and clear the security configuration of the NVS flash partition using the `nvs_bootloader_secure_deinit` API.
...
You can find code examples in `storage/nvs/nvs_bootloader` (in the `storage/nvs` directory of ESP-IDF examples).
