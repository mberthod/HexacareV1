<!-- Source: _sources/api-reference/network/esp_netif_driver.rst.txt (ESP-IDF v6.0 documentation) -->

# ESP-NETIF Developer's manual

As shown in the `esp-netif structure` diagram, ESP-NETIF is in fact an intermediary between the I/O driver and the TCP/IP stack. This manual describes customization of these two sides, that is if you need to implement `esp_netif_custom_driver` or if you need to employ `esp_netif_tcpip_stack`.
...
According to the diagram shown in the `esp-netif structure` part, the following three API functions for the packet data path must be defined for connecting with ESP-NETIF:
...
It is also possible to build ESP-IDF without lwIP, please refer to `components/esp_netif_stack/README.md`.

## API Reference

The following API reference outlines these network stack interaction with the ESP-NETIF:

inc/esp_netif_net_stack.inc
