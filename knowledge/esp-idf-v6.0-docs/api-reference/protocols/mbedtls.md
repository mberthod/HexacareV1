<!-- Source: _sources/api-reference/protocols/mbedtls.rst.txt (ESP-IDF v6.0 documentation) -->
...
<td><ul>
<li>TLS 1.2 &amp; 1.3 support</li>
<li>Certificate bundle enabled</li>
<li>Hardware acceleration</li>
<li>Full cipher suite support</li>
</ul></td>
</tr>
<tr class="even">
...
<td><ul>
<li>TLS 1.2 client only</li>
<li>RSA &amp; PSK key exchange</li>
<li>AES-128 CBC/CTR modes</li>
<li>Basic X.509 parsing</li>
</ul></td>
</tr>
<tr class="odd">
<td><strong>Bluetooth (BT)</strong></td>
<td>Bluetooth applications</td>
<td><ul>
<li>Optimized for Bluetooth LE security</li>
<li>ECC P-256 curve support</li>
<li>Minimal TLS overhead</li>
<li>Bluetooth-specific algorithms</li>
</ul></td>
</tr>
</tbody>
</table>
...
Examples in ESP-IDF use `/api-reference/protocols/esp_tls` which provides a simplified API interface for accessing the commonly used TLS functionality.
...
The Mbed TLS configuration system supports preset configurations. Following is a brief list of important config options accessible at `Component Config` \> `mbedTLS`. The full list of config options can be found `here <CONFIG_MBEDTLS_MEM_ALLOC_MODE>`.
...
- `CONFIG_MBEDTLS_CERTIFICATE_BUNDLE`: Support for trusted root certificate bundle (more about this: `/api-reference/protocols/esp_crt_bundle`)
...
|----------------------------------|-----------------------------------------------------------------------------------------------------------------|----------------------|
| Default                          | NA                                                                                                              | 42196 B              |
| Enable SSL Dynamic Buffer Length | `CONFIG_MBEDTLS_SSL_VARIABLE_BUFFER_LENGTH`                                                                     | 42120 B              |
...
Under `Component Config` \> `mbedTLS`, several Mbed TLS features are enabled by default. These can be disabled if not needed to save code size. More information is available in the `Minimizing Binary Size <minimizing_binary_mbedtls>` documentation.
