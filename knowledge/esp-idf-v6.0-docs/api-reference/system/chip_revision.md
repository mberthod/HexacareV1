<!-- Source: _sources/api-reference/system/chip_revision.rst.txt (ESP-IDF v6.0 documentation) -->

# Chip Revision

## Overview

{IDF_TARGET_NAME} may have different revisions. These revisions mainly fix some issues, and sometimes also bring new features to the chip. `versioning scheme` describes the versioning of these chip revisions, and the APIs to read the versions at runtime.
...
`revision_limitation` describes how the application can specify its chip revision requirements, and the way ESP-IDF checks the compatibility. After that, there is troubleshooting information for this mechanism.
...
- Ignore maximum wafer revision (`DISABLE_WAFER_VERSION_MAJOR` eFuse). See `revision_limitation` on how this is used.
...
- Ignore maximum efuse block revision (`DISABLE_BLK_VERSION_MAJOR` eFuse). See `revision_limitation` on how this is used.
...
The minimum chip revision can be configured via the `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN` option. Specifying the minimum chip revision will limit the software to only run on a chip revisions that are high enough to support some features or bugfixes.
...
- Lower the `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN` value and rebuild the image so that it is compatible with the chip revision being used.
...
For the chip revision, the second stage bootloader and the application binary images contain the `esp_image_header_t` header, which stores information specifying the chip revisions that the image is permitted to run on. This header has 3 fields related to the chip revisions:

- `min_chip_rev` - Minimum chip MAJOR revision required by image (but for ESP32-C3 it is MINOR revision). Its value is determined by `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN`.
- `min_chip_rev_full` - Minimum chip MINOR revision required by image in format: `major * 100 + minor`. Its value is determined by `CONFIG_{IDF_TARGET_CFG_PREFIX}_REV_MIN`.
...
1.  The first stage (ROM) bootloader does not check minimum and maximum revision fields from `esp_image_header_t` before running the 2nd stage bootloader.
...
<!-- Only for: esp32 or esp32c3 or esp32s2 or esp32s3 -->
...
{IDF_TARGET_NAME} chip support was added in ESP-IDF v4.2. {IDF_TARGET_NAME} chips have `rev_min` in `esp_image_header_t` header set to `0` because `Minimum Supported ESP32-S2 Revision` Kconfig option was not introduced, which means that the old bootloader does not check the chip revision. Any app can be loaded by such bootloader in range `v0.0` to `v3.15`.

Please check the chip version using `esptool chip-id` command.

## References

- [Compatibility Advisory for Chip Revision Numbering Scheme](https://www.espressif.com.cn/sites/default/files/advisory_downloads/AR2022-005%20Compatibility%20Advisory%20for%20Chip%20Revision%20Numbering%20%20Scheme.pdf)
- [Compatibility Between ESP-IDF Releases and Revisions of Espressif SoCs](https://github.com/espressif/esp-idf/blob/master/COMPATIBILITY.md)
- [SoC Errata](https://www.espressif.com.cn/en/support/documents/technical-documents?keys=errata)
- `/versions`
