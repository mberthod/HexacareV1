<!-- Source: _sources/api-reference/system/power_management.rst.txt (ESP-IDF v6.0 documentation) -->
...
Automatic Light-sleep is based on FreeRTOS Tickless Idle functionality. If automatic Light-sleep is requested while the option `CONFIG_FREERTOS_USE_TICKLESS_IDLE` is not enabled in menuconfig, `esp_pm_configure` will return the error <span class="title-ref">ESP_ERR_NOT_SUPPORTED</span>.
...
| Lock                    | Description                                                                                                                                               |
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| `ESP_PM_CPU_FREQ_MAX`   | Requests CPU frequency to be at the maximum value set with `esp_pm_configure`. For {IDF_TARGET_NAME}, this value can be set to {IDF_TARGET_MAX_CPU_FREQ}. |
...
\- **Ethernet**: between calls to `esp_eth_driver_install` and `esp_eth_driver_uninstall`. :SOC_WIFI_SUPPORTED: - **WiFi**: between calls to `esp_wifi_start` and `esp_wifi_stop`. If modem sleep is enabled, the lock will be released for the periods of time when radio is disabled. :SOC_TWAI_SUPPORTED: - **TWAI**: between calls to `twai_driver_install` and `twai_driver_uninstall` (only when the clock source is set to `TWAI_CLK_SRC_APB`). :SOC_BT_SUPPORTED and esp32: - **Bluetooth**: between calls to `esp_bt_controller_enable` and `esp_bt_controller_disable`. If Bluetooth Modem-sleep is enabled, the `ESP_PM_APB_FREQ_MAX` lock will be released for the periods of time when radio is disabled. However the `ESP_PM_NO_LIGHT_SLEEP` lock will still be held, unless `CONFIG_BTDM_CTRL_LOW_POWER_CLOCK` option is set to "External 32 kHz crystal". :SOC_BT_SUPPORTED and not esp32: - **Bluetooth**: between calls to `esp_bt_controller_enable` and `esp_bt_controller_disable`. If Bluetooth Modem-sleep is enabled, the `ESP_PM_APB_FREQ_MAX` lock will be released for the periods of time when radio is disabled. However the `ESP_PM_NO_LIGHT_SLEEP` lock will still be held. :SOC_PCNT_SUPPORTED: - **PCNT**: between calls to `pcnt_unit_enable` and `pcnt_unit_disable`. :SOC_SDM_SUPPORTED: - **Sigma-delta**: between calls to `sdm_channel_enable` and `sdm_channel_disable`. :SOC_MCPWM_SUPPORTED: - **MCPWM**: between calls to `mcpwm_timer_enable` and `mcpwm_timer_disable`, as well as `mcpwm_capture_timer_enable` and `mcpwm_capture_timer_disable`.
