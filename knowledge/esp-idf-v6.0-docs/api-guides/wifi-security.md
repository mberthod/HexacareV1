<!-- Source: _sources/api-guides/wifi-security.rst.txt (ESP-IDF v6.0 documentation) -->
...
- Additional cipher suites supported by mbedtls when the <span class="title-ref">USE_MBEDTLS_CRYPTO</span> flag is enabled
...
- WPA3-Enterprise 192-bit mode is supported only on modules that have `SOC_WIFI_GCMP_SUPPORT`.
- To enable WPA3-Enterprise 192-bit mode, configure `CONFIG_ESP_WIFI_SUITE_B_192`.
...
- **EAP-FAST**
  ...
  An authentication method based on Protected Access Credentials (PAC), requiring an identity and password. To use EAP-FAST, `CONFIG_ESP_WIFI_MBEDTLS_TLS_CLIENT` must be **disabled**.
...
- `wifi/wifi_enterprise`
  ...
  Demonstrates connecting {IDF_TARGET_NAME} using EAP-TLS, EAP-PEAP, and EAP-TTLS. For details on generating certificates with OpenSSL and running the example, refer to `wifi/wifi_enterprise/README.md`.
...
After configuring all required setting for WPA3-Personal SoftAP, application developers have to set `WIFI_AUTH_WPA3_PSK` for <span class="title-ref">authmode</span> in `wifi_ap_config_t` to start AP in WPA3 security. SoftAP can be also configured to use `WIFI_AUTH_WPA2_WPA3_PSK` mixed mode.
