<!-- Source: _sources/api-guides/tools/idf-docker-image.rst.txt (ESP-IDF v6.0 documentation) -->
...
Commands which communicate with the development board, such as `idf.py flash` and `idf.py monitor` does not work in the container, unless the serial port is passed through into the container. This can be done with Docker for Linux with the [device option](https://docs.docker.com/engine/reference/run/#runtime-privilege-and-linux-capabilities). However, currently, this is not possible with Docker for Windows (<https://github.com/docker/for-win/issues/1018>) and Docker for Mac (<https://github.com/docker/for-mac/issues/900>). This limitation may be overcome by using [remote serial ports](https://docs.espressif.com/projects/esptool/en/latest/esptool/remote-serial-ports.html). An example of how to do this can be found in the following [using remote serial port](#using-remote-serial-port) section.

> **Note**
>
> ### Using Remote Serial Port

The [RFC2217](http://www.ietf.org/rfc/rfc2217.txt) (Telnet) protocol can be used to remotely connect to a serial port. For more information please see the [remote serial ports](https://docs.espressif.com/projects/esptool/en/latest/esptool/remote-serial-ports.html) documentation in the ESP tool project. This method can also be used to access the serial port inside a Docker container if it cannot be accessed directly. Following is an example of how to use the Flash command from within a Docker container.

On host install and start `esp_rfc2217_server`:

- On Windows, the package is available as a one-file bundled executable created by `pyinstaller` and it can be downloaded from the [esptool releases](https://github.com/espressif/esptool/releases) page in a ZIP archive along with other ESP tool utilities:
...
- [Using Docker Container](https://docs.espressif.com/projects/vscode-esp-idf-extension/en/latest/additionalfeatures/docker-container.html)
